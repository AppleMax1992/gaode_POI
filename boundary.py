...
#TODO
# 高德上申请的key
key = '8dca971e5198373566d90726079ad0b0'
#搜素的城市名(全名)
addr_name = '苏州'
...
def getlnglat(address):
    ...
    uri = url + 'keywords=' + quote(address) + '&key=' + key + '&subdistrict=0' + '&extensions=all'
...
num = 0
ad = getSubName(addr_name)  # 得到上海下属区域的城市代码
add = getlnglat(addr_name)  # 得到上海整个的边界数据
while num < len(ad):
    add = pd.concat([add, getlnglat(ad[num].encode("utf-8"))])  # 得到上海下属的全部区域的边界数据
    num += 1
add.to_csv('{0}.csv'.format(addr_name), encoding='utf_8_sig')